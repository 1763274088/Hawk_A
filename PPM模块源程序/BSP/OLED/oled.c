...
const unsigned char F8X16[] =
{
	...
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
	0x01//0x00,0x06,0x01,0x01,0x02,0x02,0x04,0x04,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//~94
...
void OLED_Init(void) {
	...
	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);// 使PA15口 可以使用普通IO口功能

	GPIO_InitTypeDef OLED_GPIO1_InitStructure;
	RCC_APB2PeriphClockCmd(OLED_RCCEN1, ENABLE);     //GPIO濮嬬粓浣胯兘

	OLED_GPIO1_InitStructure.GPIO_Pin = OLED_DC;
	OLED_GPIO1_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	OLED_GPIO1_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(OLED_GPIO1, &OLED_GPIO1_InitStructure);                   //閰嶇疆OLED绔彛寮曡剼涓篏PIO杈撳嚭

	GPIO_InitTypeDef OLED_GPIO2_InitStructure;
	RCC_APB2PeriphClockCmd(OLED_RCCEN2, ENABLE);     //GPIO濮嬬粓浣胯兘
	OLED_GPIO2_InitStructure.GPIO_Pin = OLED_SCL | OLED_SDA | OLED_RST;
	OLED_GPIO2_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	OLED_GPIO2_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(OLED_GPIO2, &OLED_GPIO2_InitStructure);
...
void Dis_Float(unsigned char Y, unsigned char X, double real, unsigned char N) {
	...
	n[5] = '.';
	n[6 + N] = '\0';
	for (j = 0; j < 5 - i_Count; j++)
	{
		n[j] = ' ';
	}
	OLED_P6x8Str(X, Y, &n[1]);
}
